...
if ($method === 'POST') {
    ...
    if (isset($data->nomor_induk) && isset($data->password_lama) && isset($data->password_baru)) {
        ...
        try {
            // Ambil data admin
            $stmt = $pdo->prepare("SELECT kata_sandi FROM pengguna WHERE nomor_induk = ? AND peran = 'admin'");
            $stmt->execute([$nim]);
            $user = $stmt->fetch(PDO::FETCH_ASSOC);

            if ($user) {
                // Cek apakah password lama cocok
                if (password_verify($pwd_lama, $user['kata_sandi'])) {
                    // Enkripsi password baru
                    $hash_baru = password_hash($pwd_baru, PASSWORD_DEFAULT);
                    
                    // Simpan ke database
                    $update = $pdo->prepare("UPDATE pengguna SET kata_sandi = ? WHERE nomor_induk = ? AND peran = 'admin'");
                    $update->execute([$hash_baru, $nim]);
                ...
                echo json_encode(["status" => "gagal", "pesan" => "Pengguna tidak ditemukan atau bukan admin."]);
            }
