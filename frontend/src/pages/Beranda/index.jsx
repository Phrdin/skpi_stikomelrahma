...
const Beranda = () => {
  ...
  const poinUas = parseInt(statistik.total_poin, 10) || 0; // Gunakan akumulasi poin total
  ...
  let statusPanel = {
    ...
    desc: `Anda masih harus mengumpulkan poin untuk memenuhi syarat UAS maupun SKPI.`
  };
  ...
  if (lulusSkpi) {
    statusPanel = {
      ...
      desc: 'Selamat! Anda telah memenuhi syarat lulus 250+ poin. Anda sekarang dapat mencetak dokumen SKPI resmi kampus.'
    };
  } else if (lulusUas) {
    statusPanel = {
      ...
      desc: `Luar biasa! Poin Anda untuk syarat UAS Semester ${semester} sudah tercapai. Silakan tunjukkan layar ini ke bagian akademik untuk verifikasi UAS.`
    };
  }
    ...
    <div className="animate-in fade-in duration-500 pb-20">
          ...
          <div className={`p-4 rounded-xl border shadow-sm flex items-center gap-4 transition-all ${lulusUas ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-blue-50 text-blue-700 border-blue-200'}`}>
            ...
            <div>
              ...
              <p className="text-sm font-bold">
                {lulusUas ? 'Memenuhi Syarat UAS!' : `Kurang ${targetUas - poinUas} Poin lagi`}
              </p>
            </div>
          </div>
        )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <CardStat 
          ...
          label={isAdmin ? "Total Mahasiswa" : "Poin Akumulasi"} 
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
                <div className="flex justify-between items-end mb-6">
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-500 mt-1">Target Poin Khusus Semester Ini: {targetUas} Poin</p>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 text-center">
                    ...
                    <p className="text-2xl font-bold text-blue-900">{targetUas} <small className="text-xs font-medium">PTS</small></p>
                  </div>
                  <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 text-center">
                    <p className="text-xs font-semibold text-indigo-600 uppercase mb-1">Total Poin</p>
                    <p className="text-2xl font-bold text-indigo-900">{poinUas} <small className="text-xs font-medium">PTS</small></p>
                  </div>
                </div>
            </div>
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
              <h3 className="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2">
                 <CheckCircle2 size={18} className="text-emerald-500"/> Syarat Minimal Poin UAS
              </h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                 {(statistik.target_semua_semester || []).map((item) => (
                   <div key={item.semester} className={`p-4 rounded-xl border flex flex-col items-center justify-center transition-all ${semester === parseInt(item.semester) ? 'border-blue-600 bg-blue-50' : 'border-gray-100 bg-gray-50'}`}>
                      ...
                      <p className={`text-lg font-bold mt-1 ${semester === parseInt(item.semester) ? 'text-blue-900' : 'text-gray-700'}`}>{item.target_poin} <small className="text-[10px] font-medium">PTS</small></p>
                   </div>
                 ))}
              </div>
            </div>
            ...
            <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
              <div className="flex justify-between items-end mb-4">
                  <div>
                    ...
                    <p className="text-sm font-medium text-gray-500">Target Penerbitan: {targetSkpi} Poin</p>
                  </div>
